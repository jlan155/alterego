...
// here we have a hook that removed the add to cart button from our loop products
remove_action('woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10);

// here we use a filter to return the full sized image for our product archive images (vs. the low res version)
add_filter( 'single_product_archive_thumbnail_size', function( $size ) {
	return 'full';
} );

// here we remove the breadcrumbs on the category page
...
// here we remove the results count and ordering
...
function get_category_image($term) {
	...
	// here we need to check we actually have a current category, if not we are going to grab the default category using the $term variable
	if (empty($category_id)) {
		// here we find the category by its slug
		$category = get_term_by('slug', $term, 'product_cat');
		// then we grab the term id from the category and overwrite tge $category_id variable
...
// here we write a function that gets us the custom background color for our product
...
// this filter removes the 'Select an option' default option with our variation select elements
...
// this filter swaps the order of our flash message around so that the button is on the right and the text on the left
